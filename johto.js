...
const getCachedData = () => {
    const cachedData = localStorage.getItem("johto");
    if (cachedData) {
...
if (cachedData) {
    ...
    fetch(
      "https://api.pokemontcg.io/v2/cards?q=nationalPokedexNumbers:[152 TO 251]")
      .then((res) => res.json())
      .then((data) => {
        ...
        localStorage.setItem("johto", JSON.stringify(cards));
        renderCards(cards);
    });
}