...
const getCachedData = () => {
    const cachedData = localStorage.getItem("alola");
    if (cachedData) {
...
if (cachedData) {
    ...
    fetch(
      "https://api.pokemontcg.io/v2/cards?q=nationalPokedexNumbers:[722 TO 809]")
      .then((res) => res.json())
      .then((data) => {
        ...
        localStorage.setItem("alola", JSON.stringify(cards));
        renderCards(cards);
    });
}