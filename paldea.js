...
const getCachedData = () => {
    const cachedData = localStorage.getItem("paldea");
    if (cachedData) {
...
if (cachedData) {
    ...
    fetch(
      "https://api.pokemontcg.io/v2/cards?q=nationalPokedexNumbers:[899 TO 1010]")
      .then((res) => res.json())
      .then((data) => {
        ...
        localStorage.setItem("paldea", JSON.stringify(cards));
        renderCards(cards);
    });
}