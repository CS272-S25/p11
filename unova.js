...
const getCachedData = () => {
    const cachedData = localStorage.getItem("unova");
    if (cachedData) {
...
if (cachedData) {
    ...
    fetch(
      "https://api.pokemontcg.io/v2/cards?q=nationalPokedexNumbers:[494 TO 649]")
      .then((res) => res.json())
      .then((data) => {
        ...
        localStorage.setItem("unova", JSON.stringify(cards));
        renderCards(cards);
    });
}
