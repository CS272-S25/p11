...
const getCachedData = () => {
    const cachedData = localStorage.getItem("kalos");
    if (cachedData) {
...
if (cachedData) {
    ...
    fetch(
      "https://api.pokemontcg.io/v2/cards?q=nationalPokedexNumbers:[650 TO 721]")
      .then((res) => res.json())
      .then((data) => {
        ...
        localStorage.setItem("kalos", JSON.stringify(cards));
        renderCards(cards);
    });
}