...
const getCachedData = () => {
    const cachedData = localStorage.getItem("sinnoh");
    if (cachedData) {
...
if (cachedData) {
    ...
    fetch(
      "https://api.pokemontcg.io/v2/cards?q=nationalPokedexNumbers:[387 TO 493]")
      .then((res) => res.json())
      .then((data) => {
        ...
        localStorage.setItem("sinnoh", JSON.stringify(cards));
        renderCards(cards);
    });
}
