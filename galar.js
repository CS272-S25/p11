...
const getCachedData = () => {
    const cachedData = localStorage.getItem("galar");
    if (cachedData) {
...
if (cachedData) {
    ...
    fetch(
      "https://api.pokemontcg.io/v2/cards?q=nationalPokedexNumbers:[810 TO 898]")
      .then((res) => res.json())
      .then((data) => {
        ...
        localStorage.setItem("galar", JSON.stringify(cards));
        renderCards(cards);
    });
}